...
my @spes=qw(Abrevicaudatus Acrassiceps Acura Amelas Apoly Cartus Cmacrodon Cquinquelineatus Daru Fthermalis Fugu Fvariegata Medaka Nfusca Nsavayensis Nviria Oangustatus Ocompressus Ocookii Ocyanosoma Odoederleini Onigrofasciatus Onotatus Onovemfasciatus Padel Pexostigma Pfraenatus Platyfish Pmirifica Pmol Rgracilis Snematoptera Stickleback Tfucata Tzosterophora Zebrafish Zleptacanthus Zviridiventer);
...
while (<ORTH>) {
        ...
        if (/^Orthogroup/) {
                ...
                #print "$j\t$k\n";
                my $fasta=$fas{$orth};
                if ($j && $j==38) { # for single copy genes
                ...
                } elsif ($k==38) { # for mutiple copy genes
                        my $tree =$tre{$orth};
                        my $output2=$output."/possvm_output";
                        system("mkdir $output2") unless -e $output2;

                        my $cmd="python /data2/jlkang/software/possvm-orthology-master/possvm.py -i $tree -o $output2";
                        system($cmd);
