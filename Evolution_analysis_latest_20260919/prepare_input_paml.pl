...
while (<INPUT>) {
        ...
        system("Gblocks pal2nal.fasta -b4=10 -b5=n -b3=5 -t=c"); # Gblocks: pal2nal.fasta-gb.fa
    ...
    foreach my $spe (@spes) {
        ...
        $i++; if ($i==1) {
                my $num=@spes; my $len=length($hash2{$spe});
                print FIL3 "$num  $len\n";
    ...
    while (<FIL3>) {
        ...
        if ($j==@spes && $len && $len>=150) {
                print FIL5 "$orth_id\n";
        }
    }
    #system("rm pal2nal.fasta-gb.fa");
    chdir "$pwd";
}
